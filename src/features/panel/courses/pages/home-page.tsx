    ...
    <section className="flex min-h-[70vh] flex-col justify-center gap-8 py-10">
      <div className="max-w-2xl space-y-4">
        ...
        <h1 className="text-4xl font-bold leading-tight tracking-tight sm:text-5xl">
          {t('panel.home.headline')}
        </h1>
        <p className="text-lg text-[var(--color-muted-foreground)]">{t('panel.home.subheadline')}</p>
        <div className="flex flex-wrap gap-3 pt-2">
          <Button asChild size="lg">
            <Link href="/panel/courses">{t('panel.home.ctaCourses')}</Link>
          </Button>
          <Button asChild size="lg" variant="outline">
            <Link href={user ? '/panel/account' : '/panel/login'}>{t('panel.home.ctaAccount')}</Link>
          </Button>
        </div>
      </div>
    </section>
