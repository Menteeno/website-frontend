  ...
  const buyMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!result?.paymentUrl) {
        throw new Error(result?.error || t("panel.payments.startFailed"));
      }
      ...
      <div className="grid gap-6 lg:grid-cols-[1.4fr_1fr]">
        <div className="space-y-4">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <Badge>{formatPrice(price, course.currency, locale)}</Badge>
            {hasSale ? (
            ...
            {enrollment ? (
              <Badge variant="secondary">{t("panel.courses.enrolled")}</Badge>
            ) : null}
            {firstFreeLesson ? (
              <Badge variant="outline">
                {t("panel.courses.hasFreePreview")}
              </Badge>
            ) : null}
          </div>
          <div className="flex flex-wrap gap-3">
            {firstFreeLesson ? (
              <Button asChild>
                <Link
                  href={`/panel/courses/${course.slug}/lessons/${firstFreeLesson.id}`}
                >
            ...
            {enrollment && firstLesson ? (
              <Button asChild variant={firstFreeLesson ? "outline" : "default"}>
                <Link
                  href={`/panel/account/courses/${course.id}/lessons/${firstLesson.id}`}
                >
                  {t("panel.courses.startLearning")}
                </Link>
              </Button>
            ) : null}
            {!enrollment && user && price <= 0 ? (
              <Button
          ...
          {course.description ? (
            <div className="max-w-none whitespace-pre-wrap text-sm leading-7">
              {course.description}
            </div>
          ) : null}
        </div>
                      ...
                      <div className="flex shrink-0 items-center gap-2">
                        ...
                        {lesson.is_free ? (
                          <Button asChild size="sm" variant="ghost">
                            <Link
                              href={`/panel/courses/${course.slug}/lessons/${lesson.id}`}
                            >
                              {t("panel.courses.watchFree")}
                            </Link>
                          </Button>
                          ...
                          <Button asChild size="sm" variant="ghost">
                            <Link
                              href={`/panel/account/courses/${course.id}/lessons/${lesson.id}`}
                            >
                              {t("panel.courses.startLearning")}
                            </Link>
                          </Button>
